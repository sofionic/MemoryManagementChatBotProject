...
{
    //// STUDENT CODE
    //// this constructor is not needed task 05 as root node
...
{
    //// STUDENT CODE
    //// so the destructor  is not needed task 03 due to unique pointer delete will be automatic
    ...
    // delete all edges  task 04 due to unique pointer delete will be automatic
...
void ChatLogic::LoadAnswerGraphFromFile(std::string filename)
{
            ...
            if (type != tokens.end())
            {
                ...
                if (idToken != tokens.end())
                {
                    ...
                    if (type->second == "NODE")
                    {
                        ...
                        //auto newNode = std::find_if(_nodes.begin(), _nodes.end(), [&id](GraphNode *node) { return node->GetID() == id; }); // Task 03 GraphNode pointer changed to unique pointer
                        ...
                        {
                           // _nodes.emplace_back(new GraphNode(id)); // task 03  Graph Node pointer is unique pointer
                    ...
                    if (type->second == "EDGE")
                    {
                        ...
                        if (parentToken != tokens.end() && childToken != tokens.end())
                        {
                            ...
                            // task 03 below 2 lines GraphNode * pointers modified to unique pointer
                            ...
                            // create new edge
                            //GraphEdge *edge = new GraphEdge(id); // task 04 commented
                            std::unique_ptr<GraphEdge> edge = std::make_unique<GraphEdge>(id); //task 04 edge is unique pointer 
                            edge->SetChildNode(childNode->get()); //task 03
                            edge->SetParentNode(parentNode->get()); //task 03
                            //_edges.push_back(edge);  // task 03 commented out due to using of unique_ptr

                            // find all keywords for current node
                            AddAllTokensToElement("KEYWORD", tokens, *edge.get());

                            // store reference in child node and parent node
                            (*childNode)->AddEdgeToParentNode(edge.get());  // task 04  used getter function
                            (*parentNode)->AddEdgeToChildNode(std::move(edge));  // task 04 used std move ownershipt to childnodes
                        }
    ...
    for (auto it = std::begin(_nodes); it != std::end(_nodes); ++it)
    {
        ...
        if ((*it)->GetNumberOfParents() == 0)
        {
            ...
            if (rootNode == nullptr)
            {
                //rootNode = *it; // assign current node to root -- task 03 using get for root node
    ...
    // create local chat bot for task 05
    ChatBot chatbot = ChatBot("../images/chatbot.png");
    chatbot.SetChatLogicHandle(this);
    // add chatbot to graph root node
    //_chatBot->SetRootNode(rootNode);-- task 05 
    chatbot.SetRootNode(rootNode);
    //rootNode->MoveChatbotHere(_chatBot); task 05 move chatbot it is an exclusive resource
